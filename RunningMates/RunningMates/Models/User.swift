...
class User: NSObject, NSCoding {
    ...
    init?(json: [String: Any]) throws {
        ...
        guard let mates = json["mates"] as! [String: Any]? else {
            throw UserInitError.invalidMates
        }
        guard let potentialMates = json["potentialMates"] as! [String: Any]? else {
            throw UserInitError.invalidPotentialMates
        }
        guard let seenProfiles = json["seenProfiles"] as! [Any]? else {
            throw UserInitError.invalidSeenProfiles
        }
        guard let blockedMates = json["blockedMates"] as! [String: Any]? else {
            throw UserInitError.invalidBlockedMates
        }
        guard let requestsReceived = json["requestsReceived"] as! [String: Any]? else {
            throw UserInitError.invalidRequestsReceived
        }
